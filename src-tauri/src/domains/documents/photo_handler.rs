...
use crate::commands::{ApiResponse, AppState, AppError};
...
impl PhotoService {
    ...
    fn create_storage_provider(settings: &PhotoStorageSettings) -> PhotoResult<StorageProvider> {
        match settings.photo_storage_type.as_str() {
            ...
            "cloud" => {
                ...
                let bucket = settings.cloud_bucket.clone().ok_or_else(|| PhotoError::Validation("Cloud bucket not configured".to_string()))?;
                let region = settings.cloud_region.clone().ok_or_else(|| PhotoError::Validation("Cloud region not configured".to_string()))?;
                Ok(StorageProvider::Cloud { provider, bucket, region })
            }
            "hybrid" => {
                ...
                let bucket = settings.cloud_bucket.clone().ok_or_else(|| PhotoError::Validation("Cloud bucket not configured".to_string()))?;
                let region = settings.cloud_region.clone().ok_or_else(|| PhotoError::Validation("Cloud region not configured".to_string()))?;
                Ok(StorageProvider::Hybrid { local_path, cloud_provider: provider, bucket, region })
            }
            _ => Err(PhotoError::Validation(format!("Unknown storage type: {}", settings.photo_storage_type))),
        }
    }
    ...
    fn parse_cloud_provider(settings: &PhotoStorageSettings) -> PhotoResult<CloudProvider> {
        match settings.cloud_provider.as_deref() {
            ...
            Some(provider) => Err(PhotoError::Validation(format!("Unknown cloud provider: {}", provider))),
            None => Err(PhotoError::Validation("Cloud provider not configured".to_string())),
        }
    }
    ...
    fn get_local_storage_path(settings: &PhotoStorageSettings) -> PhotoResult<PathBuf> {
        match &settings.local_storage_path {
            ...
            None => {
                let mut path = std::env::current_dir().map_err(|e| PhotoError::Storage(format!("Could not determine current directory: {}", e)))?;
                path.push("photos");
                Ok(path)
            }
        }
    ...
    fn validate_store_request(&self, request: &StorePhotoRequest) -> PhotoResult<()> {
        if request.intervention_id.is_empty() {
            return Err(PhotoError::Validation("Intervention ID is required".to_string()));
        }
        Ok(())
    }
    ...
    fn validate_image_data(&self, data: &[u8]) -> PhotoResult<()> {
        ...
        if data.len() > self.max_file_size {
            return Err(PhotoError::FileTooLarge(format!("Size {} exceeds limit", data.len())));
        }
        Ok(())
    }

    pub async fn store_photo(&self, request: StorePhotoRequest, image_data: Vec<u8>) -> PhotoResult<StorePhotoResponse> {
        ...
        let (file_path, storage_url) = match &self.storage_provider {
            StorageProvider::Local => {
                let file_path = self.store_locally(&request.intervention_id, &request.file_name, &compressed_data).await?;
                (file_path.clone(), Some(format!("file://{}", file_path.display())))
            }
            StorageProvider::Cloud { provider, bucket, region } => {
                let storage_url = self.store_in_cloud(provider, bucket, region, &request.intervention_id, &request.file_name, &compressed_data).await?;
                let file_path = self.generate_local_cache_path(&request.intervention_id, &request.file_name);
                (file_path, Some(storage_url))
            }
            StorageProvider::Hybrid { local_path, cloud_provider, bucket, region } => {
                let file_path = self.store_locally_with_path(local_path, &request.intervention_id, &request.file_name, &compressed_data).await?;
                let storage_url = self.store_in_cloud(cloud_provider, bucket, region, &request.intervention_id, &request.file_name, &compressed_data).await?;
                (file_path, Some(storage_url))
            }
        };
        ...
        let (width, height) = self.extract_image_dimensions(&compressed_data)?;
        let (quality_score, blur_score, exposure_score, composition_score) = self.calculate_photo_quality_scores(&compressed_data)?;

        let mut photo = Photo::new(request.intervention_id.clone(), file_path.to_string_lossy().to_string());
        ...
        photo.photo_type = request.photo_type.as_ref().and_then(|s| serde_json::from_str(&format!(r#""{}""#, s)).ok());
        photo.photo_category = request.photo_category.as_ref().and_then(|s| serde_json::from_str(&format!(r#""{}""#, s)).ok());
    ...
    async fn store_locally(&self, intervention_id: &str, file_name: &str, data: &[u8]) -> PhotoResult<PathBuf> {
        let file_path = self.generate_file_path(intervention_id, file_name);
        self.write_file_atomic(&file_path, data).await?;
        Ok(file_path)
    }

    async fn store_locally_with_path(&self, base_path: &Path, intervention_id: &str, file_name: &str, data: &[u8]) -> PhotoResult<PathBuf> {
        let file_path = base_path.join(intervention_id).join("photos").join(file_name);
        self.write_file_atomic(&file_path, data).await?;
        Ok(file_path)
    }
    ...
    async fn store_in_cloud(&self, provider: &CloudProvider, bucket: &str, region: &str, intervention_id: &str, file_name: &str, data: &[u8]) -> PhotoResult<String> {
        match provider {
            CloudProvider::AwsS3 => {
    ...
    pub fn generate_file_path(&self, intervention_id: &str, file_name: &str) -> PathBuf {
        self.local_storage_path.join(intervention_id).join("photos").join(file_name)
    }

    pub fn generate_local_cache_path(&self, intervention_id: &str, file_name: &str) -> PathBuf {
        self.local_storage_path.join("cache").join(intervention_id).join(file_name)
    }
    ...
    pub fn get_photos(&self, request: GetPhotosRequest) -> PhotoResult<GetPhotosResponse> {
        ...
        let sql = format!("SELECT * FROM photos {} ORDER BY created_at DESC LIMIT ? OFFSET ?", where_clause);
        let mut query_params = params_vec.clone();
        query_params.push(limit.to_string());
        query_params.push(offset.to_string());

        let rusql_params: Vec<&dyn rusqlite::ToSql> = query_params.iter().map(|s| s as &dyn rusqlite::ToSql).collect();
        let photos = self.db.query_as::<Photo>(&sql, &rusql_params[..])?;

        let count_sql = format!("SELECT COUNT(*) FROM photos {}", where_clause);
        let count_rusql_params: Vec<&dyn rusqlite::ToSql> = params_vec.iter().map(|s| s as &dyn rusqlite::ToSql).collect();
        let total: i32 = self.db.query_single_value(&count_sql, &count_rusql_params[..])?;

        Ok(GetPhotosResponse { photos, total })
    }

    pub fn get_photo(&self, id: &str) -> PhotoResult<Option<Photo>> {
        Ok(self.db.query_single_as::<Photo>("SELECT * FROM photos WHERE id = ?", params![id])?)
    }

    pub fn delete_photo(&self, id: &str) -> PhotoResult<()> {
        let photo = self.get_photo(id)?.ok_or_else(|| PhotoError::NotFound(format!("Photo {} not found", id)))?;
        if Path::new(&photo.file_path).exists() {
            let _ = std::fs::remove_file(&photo.file_path);
        }
        self.db.execute("DELETE FROM photos WHERE id = ?", params![id])?;
        Ok(())
    }

    pub fn update_photo_metadata(&self, id: &str, updates: PhotoMetadataUpdate) -> PhotoResult<Photo> {
        let mut photo = self.get_photo(id)?.ok_or_else(|| PhotoError::NotFound(format!("Photo {} not found", id)))?;

        if let Some(title) = updates.title { photo.title = title; }
        if let Some(description) = updates.description { photo.description = description; }
        if let Some(notes) = updates.notes { photo.notes = notes; }
        if let Some(is_approved) = updates.is_approved { photo.is_approved = is_approved; }
        if let Some(approved_by) = updates.approved_by { photo.approved_by = approved_by; }
        if let Some(rejection_reason) = updates.rejection_reason { photo.rejection_reason = rejection_reason; }
    ...
    fn save_photo_record(&self, photo: &Photo) -> PhotoResult<()> {
        let exists: i32 = self.db.query_single_value("SELECT COUNT(*) FROM photos WHERE id = ?", params![photo.id])?;
        if exists > 0 {
            self.db.execute(
    ...
    pub async fn compress_image_if_needed(&self, data: Vec<u8>) -> PhotoResult<Vec<u8>> {
        if data.len() <= self.max_file_size { return Ok(data); }
        let _permit = self.processing_semaphore.clone().acquire_owned().await.map_err(|e| PhotoError::Processing(format!("Semaphore error: {}", e)))?;
        let quality = self.jpeg_quality;
        let max_size = self.max_file_size;
        tokio::task::spawn_blocking(move || Self::compress_image_blocking(&data, quality, max_size)).await.map_err(|e| PhotoError::Processing(e.to_string()))?
    }

    fn compress_image_blocking(data: &[u8], quality: u8, max_size: usize) -> PhotoResult<Vec<u8>> {
        let img = image::load_from_memory(data).map_err(|e| PhotoError::Processing(e.to_string()))?;
        let mut output = Cursor::new(Vec::new());
        let encoder = image::codecs::jpeg::JpegEncoder::new_with_quality(&mut output, quality);
        encoder.write_image(img.as_bytes(), img.width(), img.height(), img.color().into()).map_err(|e| PhotoError::Processing(e.to_string()))?;
        let compressed = output.into_inner();
        if compressed.len() > max_size && quality > 20 {
            return Self::compress_image_blocking(data, quality / 2, max_size);
        }
        Ok(compressed)
    }

    pub fn extract_image_dimensions(&self, data: &[u8]) -> PhotoResult<(Option<i32>, Option<i32>)> {
    ...
    pub fn calculate_photo_quality_scores(&self, data: &[u8]) -> PhotoResult<(Option<i32>, Option<i32>, Option<i32>, Option<i32>)> {
        match image::load_from_memory(data) {
            Ok(img) => {
                let gray = img.to_luma8();
                let (w, h) = gray.dimensions();
                let blur = self.calculate_blur_score(&gray, w, h);
                let exposure = self.calculate_exposure_score(&gray);
                let composition = self.calculate_composition_score(&gray, w, h);
                let overall = ((blur as f32 * 0.4) + (exposure as f32 * 0.3) + (composition as f32 * 0.3)) as i32;
    ...
    fn calculate_blur_score(&self, gray: &image::GrayImage, w: u32, h: u32) -> i32 {
        ...
        if count > 0 { ((variance / count as f32) / 1000.0).min(1.0) as i32 * 100 } else { 50 }
    }

    fn calculate_exposure_score(&self, gray: &image::GrayImage) -> i32 {
        let mut hist = [0u32; 256];
        for p in gray.pixels() { hist[p.0[0] as usize] += 1; }
    ...
    fn calculate_composition_score(&self, gray: &image::GrayImage, w: u32, h: u32) -> i32 {
        let tw = w / 3; let th = h / 3;
        let mut thirds_pixels = 0; let mut total_important = 0;
        for y in 0..h {
            for x in 0..w {
                let val = gray.get_pixel(x, y).0[0];
                if !(64..=192).contains(&val) {
                    total_important += 1;
                    let near_v = (x as i32 - tw as i32).abs() < 10 || (x as i32 - (tw * 2) as i32).abs() < 10;
                    let near_h = (y as i32 - th as i32).abs() < 10 || (y as i32 - (th * 2) as i32).abs() < 10;
                    if near_v || near_h { thirds_pixels += 1; }
                }
            }
        }
        if total_important > 0 { (thirds_pixels as f32 / total_important as f32 * 100.0) as i32 } else { 50 }
    }

    pub async fn generate_thumbnail(&self, data: &[u8], path: &Path) -> PhotoResult<PathBuf> {
        let data = data.to_vec();
        let thumb_path = self.thumbnail_path(path);
        let target = thumb_path.clone();
        tokio::task::spawn_blocking(move || Self::generate_thumbnail_blocking(&data, &target)).await.map_err(|e| PhotoError::Processing(e.to_string()))??;
        Ok(thumb_path)
    }
    ...
    fn generate_thumbnail_blocking(data: &[u8], path: &Path) -> PhotoResult<()> {
        let img = image::load_from_memory(data).map_err(|e| PhotoError::Processing(e.to_string()))?;
        ...
        thumb.save_with_format(&tmp, format).map_err(|e| PhotoError::Processing(e.to_string()))?;
        std::fs::rename(&tmp, path).map_err(|e| { let _ = std::fs::remove_file(&tmp); PhotoError::from(e) })?;
        Ok(())
    }
    ...
    pub fn get_stats(&self) -> PhotoResult<PhotoStats> {
        let total: i32 = self.db.query_single_value("SELECT COUNT(*) FROM photos", [])?;
        let size: i64 = self.db.query_single_value("SELECT COALESCE(SUM(file_size), 0) FROM photos", [])?;
    ...
    pub fn read_photo_data(&self, id: &str) -> PhotoResult<Vec<u8>> {
        let photo = self.get_photo(id)?.ok_or_else(|| PhotoError::NotFound(format!("Photo {} not found", id)))?;
        if !Path::new(&photo.file_path).exists() {
            return Err(PhotoError::NotFound("Photo file not found on disk".to_string()));
        }
        std::fs::read(&photo.file_path).map_err(PhotoError::Io)
    }

    pub async fn upload_photo_with_retry(&self, photo_id: &str, max_retries: u32) -> PhotoResult<()> {
        let mut photo = self.get_photo(photo_id)?.ok_or_else(|| PhotoError::NotFound(format!("Photo {} not found", photo_id)))?;
        if photo.synced { return Ok(()); }
        ...
        Err(PhotoError::Storage(format!("Upload failed after {} retries", max_retries)))
    }

    async fn attempt_upload(&self) -> PhotoResult<()> {
        use rand::Rng;
        if rand::thread_rng().gen::<f64>() < 0.7 { Ok(()) } else { Err(PhotoError::Storage("Simulated upload failure".to_string())) }
    }
}
...
pub async fn document_store_photo(
    ...
    match facade.execute(DocumentsCommand::StorePhoto { request, image_data }, &user, &get_services(&state)).await? {
...
pub async fn document_get_photos(
    ...
    let user = ctx.auth.to_user_session();
    let facade = DocumentsFacade::new(state.photo_service.clone());
    match facade.execute(DocumentsCommand::GetPhotos { request }, &user, &get_services(&state)).await? {
        DocumentsResponse::Photos(res) => Ok(ApiResponse::success(res).with_correlation_id(Some(ctx.correlation_id))),
        _ => Err(AppError::Internal("Unexpected response".into())),
    }
}
...
pub async fn document_get_photo(
    ...
    let user = ctx.auth.to_user_session();
    let facade = DocumentsFacade::new(state.photo_service.clone());
    match facade.execute(DocumentsCommand::GetPhoto { photo_id }, &user, &get_services(&state)).await? {
        DocumentsResponse::OptionalPhoto(photo) => Ok(ApiResponse::success(photo).with_correlation_id(Some(ctx.correlation_id))),
        _ => Err(AppError::Internal("Unexpected response".into())),
    }
}
...
pub async fn document_delete_photo(
    ...
    let user = ctx.auth.to_user_session();
    let facade = DocumentsFacade::new(state.photo_service.clone());
    match facade.execute(DocumentsCommand::DeletePhoto { photo_id }, &user, &get_services(&state)).await? {
        DocumentsResponse::Unit => Ok(ApiResponse::success(()).with_correlation_id(Some(ctx.correlation_id))),
        _ => Err(AppError::Internal("Unexpected response".into())),
    }
}
...
pub async fn document_get_photo_data(
    ...
    let user = ctx.auth.to_user_session();
    let facade = DocumentsFacade::new(state.photo_service.clone());
    match facade.execute(DocumentsCommand::GetPhotoData { photo_id }, &user, &get_services(&state)).await? {
        DocumentsResponse::PhotoData(data) => Ok(ApiResponse::success(data).with_correlation_id(Some(ctx.correlation_id))),
        _ => Err(AppError::Internal("Unexpected response".into())),
    }
}
...
pub async fn document_update_photo_metadata(
    ...
    let user = ctx.auth.to_user_session();
    let facade = DocumentsFacade::new(state.photo_service.clone());
    match facade.execute(DocumentsCommand::UpdatePhotoMetadata { photo_id, updates }, &user, &get_services(&state)).await? {
        DocumentsResponse::Photo(photo) => Ok(ApiResponse::success(photo).with_correlation_id(Some(ctx.correlation_id))),
        _ => Err(AppError::Internal("Unexpected response".into())),
    }
}
...
pub async fn export_intervention_report(
    ...
    let user = ctx.auth.to_user_session();
    let facade = DocumentsFacade::new(state.photo_service.clone());
    match facade.execute(DocumentsCommand::ExportInterventionReport { intervention_id }, &user, &get_services(&state)).await? {
        DocumentsResponse::Report(res) => Ok(ApiResponse::success(res).with_correlation_id(Some(ctx.correlation_id))),
        _ => Err(AppError::Internal("Unexpected response".into())),
    }
}
...
pub async fn save_intervention_report(
    ...
    let user = ctx.auth.to_user_session();
    let facade = DocumentsFacade::new(state.photo_service.clone());
    match facade.execute(DocumentsCommand::SaveInterventionReport { intervention_id, file_path }, &user, &get_services(&state)).await? {
        DocumentsResponse::SavedPath(path) => Ok(ApiResponse::success(path).with_correlation_id(Some(ctx.correlation_id))),
        _ => Err(AppError::Internal("Unexpected response".into())),
    }
}
