//! Shared Photo contract types.
//!
//! `Photo` and its associated enums are used by both the `documents` domain
//! (storage / metadata) and the `interventions` domain (workflow photos).
//! Defining them here avoids a direct cross-domain dependency.
