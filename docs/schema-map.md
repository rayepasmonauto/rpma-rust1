...
> ADR-010 note: this repository no longer has a `001_initial_schema.sql` file. Fresh databases are bootstrapped from `src-tauri/src/db/schema.sql`, and then numbered migrations in `src-tauri/migrations/` are applied in order.
...
- Logical groupings: core auth, workflow, inventory, messaging, audit/cache, calendar/system
...
| clients | Pre-numbered bootstrap schema | `src-tauri/src/db/schema.sql` | Core entities |
...
| analytics_kpis | `025_add_analytics_dashboard.sql` | Added by migration only | Audit/analytics |
| analytics_metrics | `025_add_analytics_dashboard.sql` | Added by migration only | Audit/analytics |
| analytics_dashboards | `025_add_analytics_dashboard.sql` | Added by migration only | Audit/analytics |
| quotes | `037_quotes.sql` | `src-tauri/src/db/schema.sql` | Quotes |
| quote_items | `037_quotes.sql` | `src-tauri/src/db/schema.sql` | Quotes |
...
| quote_attachments | `047_add_quotes_missing_columns.sql` | `src-tauri/src/db/schema.sql` | Quotes |
...
- If a table is marked **Added by migration only**, it is still introduced solely by the numbered migration and is not yet folded into the bootstrap snapshot.
