...
const ProfileSettingsTab = dynamic(
  () =>
    import("./ProfileSettingsTab").then((mod) => ({
      default: mod.ProfileSettingsTab,
    })),
  {
    loading: () => (
...
const PreferencesTab = dynamic(
  () =>
    import("./PreferencesTab").then((mod) => ({ default: mod.PreferencesTab })),
  {
    loading: () => (
      <div className="flex items-center justify-center p-8">
        <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-primary"></div>
...
const SecurityTab = dynamic(
  () => import("./SecurityTab").then((mod) => ({ default: mod.SecurityTab })),
  {
    loading: () => (
      <div className="flex items-center justify-center p-8">
        <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-primary"></div>
...
const OrganizationSettingsTab = dynamic(
  () =>
    import("./OrganizationSettingsTab").then((mod) => ({
      default: mod.OrganizationSettingsTab,
    })),
  {
    loading: () => (
      <div className="flex items-center justify-center p-8">
...
const SystemSettingsTabLazy = dynamic(
  () =>
    import("@/domains/admin").then((mod) => ({
      default: mod.SystemSettingsTab,
    })),
  {
...
const SecurityPoliciesTabLazy = dynamic(
  () =>
    import("@/domains/admin").then((mod) => ({
      default: mod.SecurityPoliciesTab,
    })),
  {
    loading: () => <LoadingState />,
  },
        ...
        actions={
          <>
            ...
            <div className="hidden lg:flex items-center gap-1 text-xs text-muted-foreground bg-muted/50 px-2 py-1 rounded">
              <kbd className="px-1.5 py-0.5 text-xs bg-border/20 rounded">
                Ctrl
              </kbd>
              <span>+</span>
              <kbd className="px-1.5 py-0.5 text-xs bg-border/20 rounded">
          ...
          >
            ...
            <div className="lg:hidden mb-4">
              <div className="bg-background/50 rounded-lg p-3 border border-border/30">
                <Sheet>
                  ...
                  <SheetContent
                    side="bottom"
                    className="h-[70vh] bg-background border-[hsl(var(--rpma-border))]"
                  >
                      ...
                      {isAdmin && (
                        <>
                          <div className="border-t border-[hsl(var(--rpma-border))] my-2 pt-2">
                            <p className="text-xs text-muted-foreground px-1 mb-2 font-medium uppercase tracking-wide">
                              Configuration
                            </p>
                          </div>
                          {tabConfig
                            .filter((tab) => tab.isConfig)
                            .map((tab, index) => {
                              const Icon = tab.icon;
                              return (
                                <Button
                    ...
                    <div className="mt-6 pt-4 border-t border-[hsl(var(--rpma-border))]">
                      <p className="text-xs text-muted-foreground text-center">
                        Utilisez Ctrl+1-{tabCount} pour naviguer rapidement
                        (desktop)
                      </p>
                    </div>
                  </SheetContent>
            ...
            <div className="mt-4 md:mt-6">
              <TabsContent value="profile" className="mt-0" forceMount>
                <Suspense
                  fallback={<LoadingState message="Chargement du profil..." />}
                >
                  <ProfileSettingsTab
                    user={user ?? undefined}
              ...
              {isAdmin && (
                <TabsContent value="organization" className="mt-0" forceMount>
                  <Suspense
                    fallback={
                      <LoadingState message="Chargement de l'atelier..." />
                    }
                  >
              ...
              {isAdmin && (
                <>
                  <TabsContent value="system" className="mt-0">
                    <Suspense
                      fallback={
                        <LoadingState message="Chargement des paramètres système..." />
                      }
                    >
                  ...
                  <TabsContent value="business" className="mt-0">
                    <Suspense
                      fallback={
                        <LoadingState message="Chargement des règles métier..." />
                      }
                    >
                  ...
                  <TabsContent value="security-policies" className="mt-0">
                    <Suspense
                      fallback={
                        <LoadingState message="Chargement des politiques de sécurité..." />
                      }
                    >
                  ...
                  <TabsContent value="integrations" className="mt-0">
                    <Suspense
                      fallback={
                        <LoadingState message="Chargement des intégrations..." />
                      }
                    >
                      <IntegrationsTabLazy />
                    </Suspense>
                  ...
                  <TabsContent value="performance" className="mt-0">
                    <Suspense
                      fallback={
                        <LoadingState message="Chargement des performances..." />
                      }
                    >
                      <PerformanceTabLazy />
                    </Suspense>
                  ...
                  <TabsContent value="monitoring" className="mt-0">
                    <Suspense
                      fallback={
                        <LoadingState message="Chargement du monitoring..." />
                      }
                    >
                      <MonitoringTabLazy />
                    </Suspense>
