      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div className="flex items-center justify-between relative">
          {steps.map((step, index) => {
            ...
            const isAvailable = canAdvanceToStep(step.id as StepType);
            ...
            return (
              <React.Fragment key={step.id}>
                ...
                >
                  <motion.div
                    ...
                    onClick={() => {
                      ...
                      router.push(`/tasks/${taskId}/workflow/ppf/${getPPFStepPath(step.id as StepType)}`);
                    }}
                    onKeyDown={(event) => {
                      if (!isAccessible) return;
                      if (event.key === 'Enter') {
                ...
                {index < steps.length - 1 && (
                  ...
                  >
                    <div className={`h-0.5 rounded-full transition-all duration-500 ${getConnectorColor(step, steps[index + 1] as Step)}`} />
