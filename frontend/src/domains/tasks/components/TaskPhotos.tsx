...
export function TaskPhotos({
  ...
  const { photos, isLoading, uploadPhoto, deletePhoto, isUploading } =
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Button
          ...
          variant="ghost"
          size="sm"
          className="h-8 text-xs hover:bg-accent/5 hover:text-accent"
        >
      ...
      {photosList.length === 0 ? (
        <div className="py-8 flex flex-col items-center justify-center border border-dashed border-border/60 rounded-xl bg-background/20 group hover:bg-background/40 transition-colors">
          ...
          <p className="text-xs text-muted-foreground font-medium">
            Aucune photo {title.toLowerCase()}
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
          {photosList.map((photo, index) => {
            const displaySrc = resolveLocalImageUrl(
              photo.storage_url || photo.file_path,
            );
            return (
              <Card
                key={photo.id}
              ...
              >
                ...
                <div className="absolute bottom-0 left-0 right-0 p-1.5 bg-gradient-to-t from-black/60 to-transparent opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition-opacity">
                  <span className="text-[10px] text-white/90 font-medium">
                    {photo.created_at
                      ? formatDate(photo.created_at as unknown as string)
                      : ""}
                  </span>
                </div>
              </Card>
  ...
  return (
    <div className="space-y-10" ref={galleryRef}>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-10">
        <PhotoSection
          title="Photos Avant"
          photosList={beforePhotos}
          type="Before"
        />
        <PhotoSection
          title="Photos Après"
          photosList={afterPhotos}
          type="After"
        />
      </div>
