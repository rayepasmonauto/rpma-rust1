...
export const organizationIpc = {
  ...
  completeOnboarding: async (data: OnboardingData): Promise<Organization> => {
    const result = await safeInvoke<Organization>(IPC_COMMANDS.COMPLETE_ONBOARDING, { data: data as unknown as JsonValue });
  ...
  get: (sessionToken: string): Promise<Organization> =>
    safeInvoke<Organization>(IPC_COMMANDS.GET_ORGANIZATION, { session_token: sessionToken }),

  update: async (sessionToken: string, data: UpdateOrganizationRequest): Promise<Organization> => {
    const result = await safeInvoke<Organization>(IPC_COMMANDS.UPDATE_ORGANIZATION, { session_token: sessionToken, data: data as unknown as JsonValue });
    invalidatePattern('organization:');
    signalMutation('organization');
    return result;
  },
  ...
  uploadLogo: async (sessionToken: string, filePath?: string, base64Data?: string): Promise<Organization> => {
    const result = await safeInvoke<Organization>(IPC_COMMANDS.UPLOAD_LOGO, {
      file_path: filePath,
  ...
  getSettings: (sessionToken: string): Promise<Record<string, string | number | boolean>> =>
    safeInvoke<Record<string, string | number | boolean>>(IPC_COMMANDS.GET_ORGANIZATION_SETTINGS, { session_token: sessionToken }),
  ...
  updateSettings: async (sessionToken: string, data: UpdateOrganizationSettingsRequest): Promise<Record<string, string | number | boolean>> => {
    const result = await safeInvoke<Record<string, string | number | boolean>>(IPC_COMMANDS.UPDATE_ORGANIZATION_SETTINGS, { session_token: sessionToken, data: data as unknown as JsonValue });
    invalidatePattern('organization:');
    signalMutation('organization');
    return result;
  },
