...
export function PhotoUpload({
  ...
  const hasActiveUploads = uploads.some((upload: { status: string }) => upload.status === 'uploading' || upload.status === 'queued');
        ...
        <div className="mt-4 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <Button
              ...
              disabled={!uploads.some((u: { status: string }) => u.status === 'completed')}
            >
              Clear completed
            </Button>
          </div>
              ...
              <div key={photo.id} className="relative group">
                ...
                {photo.uploaded_at && (
                  <div className="absolute bottom-1 left-1 bg-black bg-opacity-75 text-white text-xs px-2 py-1 rounded">
                     {new Date(photo.uploaded_at as unknown as string).toLocaleDateString()}
                  </div>
                )}
              </div>
                );
