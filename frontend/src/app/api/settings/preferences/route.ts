...
export const GET = withAuth(async (request: NextRequestWithUser) => {
  ...
  if (!result.success || !result.data) {
    return settingsError(result.error || 'Failed to get preferences');
  }

  return settingsSuccess({ preferences: result.data.preferences });
}, 'all');
...
export const PUT = withAuth(async (request: NextRequestWithUser) => {
  ...
  try {
    ...
    const result = await settingsService.updatePreferences(user.id, body, token);

    if (!result.success || !result.data) {
      return settingsError(result.error || 'Failed to update preferences');
    }
