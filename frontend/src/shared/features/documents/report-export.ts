...
import { showSaveDialog } from "@/lib/ipc/platform";
    ...
    for (let attempt = 1; attempt <= maxRetries + 1; attempt += 1) {
      try {
        ...
        const result = await safeInvoke<InterventionReportResult>(
          IPC_COMMANDS.EXPORT_INTERVENTION_REPORT,
          {
        ...
        if (attempt > maxRetries) {
          return {
            success: false,
            error:
              error instanceof Error ? error.message : "Erreur export rapport",
            status: 500,
          };
        }
  ...
  async saveInterventionReport(
    ...
    try {
      ...
      if (!filePath) {
        return {
          success: false,
          error: "Aucun chemin de fichier selectionne",
          status: 400,
        };
      }

      const result = await safeInvoke<string>(
        IPC_COMMANDS.SAVE_INTERVENTION_REPORT,
        {
          interventionId,
          filePath,
      ...
      return {
        success: false,
        error:
          error instanceof Error ? error.message : "Erreur sauvegarde rapport",
        status: 500,
      };
    }
  },
